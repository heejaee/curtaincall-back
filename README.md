...
관리자가 특가상품을 등록하면 MySQL에 저장한 후, 해당 데이터를 Redis에 캐싱하도록 처리하였습니다.

### 2. 스케줄러
스케줄러 서버는 Redis 캐시를 공유하여, 매일 자정에 만료된 특가상품을 삭제하고, 당일 시작되는 특가상품을 자동 승인합니다.
* 기간이 5일 남은 특가상품은 타임딜시간이 보여집니다.
